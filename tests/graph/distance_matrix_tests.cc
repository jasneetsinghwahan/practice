#include "src/graph/distance_matrix.h"
...
// Tests bellmanfordRecursive
TEST(BellmanfordRecursive, FloydWarshallRecursive3) {
  std::vector<std::vector<int>> distances;
  auto INF = INT_MAX;
  {
    distances = {
    ...
    //auto start_time_no_memo = std::chrono::high_resolution_clock::now();
    //std::vector<long> actual_no_memo = g.BellmanFordRecursive(0);
    std::vector<long> actual_no_memo = g.BellmanFord2D(0);
    //auto end_time_no_memo = std::chrono::high_resolution_clock::now();
    //auto duration_no_memo =
    //    std::chrono::duration_cast<std::chrono::milliseconds>(end_time_no_memo - start_time_no_memo);
